...
```
User → Chọn nhiều thiết bị (theo category hoặc danh sách ID)
     → Chọn loại sự kiện
...
- [x] Tạo API endpoint `/api/device-reminder-plans/bulk`
- [x] Tạo API endpoint `/api/device-reminder-plans/upcoming`
- [x] Tạo API endpoint `/api/events/batch/:batchId`
- [x] Tạo API endpoint `/api/events/maintenance-batches`
- [x] Tạo API endpoint `/api/devices/:id/events`
- [x] Tạo script `scripts/check-maintenance-reminders.ts`
...
- [Hướng Dẫn Sử Dụng](./MAINTENANCE_SYSTEM_USAGE.md) - Hướng dẫn chi tiết cách sử dụng hệ thống
